...
def index(request, page_num = 1):
	...
	if int(page_num) != int(page.number):
		return redirect('askMe_app:index', page.number)

	return render(request, 'askMe_app/index.html', {
			'page': page,
...
def hot(request, page_num = 1):
	...
	if int(page_num) != int(page.number):
		return redirect('askMe_app:hot', page.number)

	return render(request, 'askMe_app/hot.html', {
			'page': page,
			'members': Profile.objects.best(),
			'tags': Tag.objects.populars(),
		})
...
def tag(request, tag_name, page_num = 1):
	...
	if int(page_num) != int(page.number):
		return redirect('askMe_app:tag', tag_name, page.number)

	return render(request, 'askMe_app/tag.html', {
			'page': page,
			'members': Profile.objects.best(),
			'tags': Tag.objects.populars(),
			'tag_name': tag_name,
...
def question(request, question_id, page_num = 1):
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			return redirect('askMe_app:question', question_id, page.paginator.num_pages + 1)
	else:
		form = AnswerForm()

	if int(page_num) != int(page.number):
		return redirect('askMe_app:question', question_id, page.number)

	return render(request, 'askMe_app/question.html', {
			'form': form,
			'question': question,
			'page': page,
			'id': question_id,
...
def ask(request):
	...
	if not user.is_authenticated():
		next_url = reverse('askMe_app:ask')
		login_url = reverse('askMe_app:login')
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			return redirect('askMe_app:question', question.id)
	else:
		form = AskForm()

	return render(request, 'askMe_app/ask.html', {
			'form': form,
			'members': Profile.objects.best(),
			'tags': Tag.objects.populars(),
		})
...
def login_view(request):
	...
	if user.is_authenticated():
		return redirect('askMe_app:index')

	next_page = request.GET.get('next')
	if next_page is None:
		next_page = 'askMe_app:index'
	...
	return render(request, 'askMe_app/login.html', {
			'form': form,
			'members': Profile.objects.best(),
			'tags': Tag.objects.populars(),
		})
...
def signup(request):
	...
	if user.is_authenticated():
		return redirect('askMe_app:login')
	...
	return render(request, 'askMe_app/signup.html', {
			'form': form,
			'members': Profile.objects.best(),
			'tags': Tag.objects.populars(),
		})
...
def settings(request):
	...
	return render(request, 'askMe_app/settings.html', {
			'form': form,
			'members': Profile.objects.best(),
			'tags': Tag.objects.populars(),
		})
...
def search(request, page_num = 1):
	...
	if int(page_num) != int(page.number):
		return redirect('askMe_app:search', page.number)
